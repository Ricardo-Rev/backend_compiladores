...
public class CompilerService : ICompilerService
{
    ...
    public async Task<CompileResponse> CompileAsync(CompileRequest request, int usuario_id, int sesion_id)
    {
        ...
        response.codigo_transpilado = request.lenguaje_destino switch
        {
        ...
        sw.Stop();
        response.tiempo_ms = (int)sw.ElapsedMilliseconds;
        var comp_id = await Persistir(request, usuario_id, sesion_id, "exito", tokens,
            new(), instrucciones, response.tiempo_ms, sim);
    ...
    private SimulacionDto GenerarSimulacion(List<InstruccionValidada> instrucciones)
    {
        ...
        foreach (var i in instrucciones.OrderBy(x => x.Orden))
        {
            ...
            {
                case "avanzar_vlts":
                    var cv = (i.ParametroN ?? 0) * 20;
                    dist += Math.Abs(cv); dur += Math.Abs(i.ParametroN ?? 0) * 2;
                    puntos.Add(new { tipo = "avanzar", valor = i.ParametroN, unidad = "vueltas", cm = cv });
                    break;
                case "avanzar_ctms":
                    dist += Math.Abs(i.ParametroN ?? 0); dur += (int)(Math.Abs(i.ParametroN ?? 0) * 0.5);
                    puntos.Add(new { tipo = "avanzar", valor = i.ParametroN, unidad = "centimetros", cm = i.ParametroN });
                    break;
                case "avanzar_mts":
                    var cm = (i.ParametroN ?? 0) * 100;
                    dist += Math.Abs(cm); dur += Math.Abs(i.ParametroN ?? 0) * 5;
                    puntos.Add(new { tipo = "avanzar", valor = i.ParametroN, unidad = "metros", cm });
                    break;
                case "girar":
                    dur += 2;
                    var dir = (i.ParametroN ?? 0) switch { 1 => "derecha", -1 => "izquierda", _ => "recto" };
                    puntos.Add(new { tipo = "girar", direccion = dir });
                    break;
                case "circulo":
                    var circ = (decimal)(2 * Math.PI * (i.ParametroR ?? 0));
                    dist += circ; dur += (i.ParametroR ?? 0) / 10;
                    puntos.Add(new { tipo = "circulo", radio = i.ParametroR });
                    break;
                case "cuadrado":
                    var lado = i.ParametroL ?? 0;
                    dist += lado * 4; dur += lado / 5;
                    puntos.Add(new { tipo = "cuadrado", lado });
                    break;
                case "rotar":
                    dur += Math.Abs(i.ParametroN ?? 0) * 3;
                    puntos.Add(new { tipo = "rotar", vueltas = i.ParametroN });
                    break;
                case "caminar":
                    dur += Math.Abs(i.ParametroN ?? 0) * 2;
                    puntos.Add(new { tipo = "caminar", pasos = i.ParametroN });
                    break;
                case "moonwalk":
                    dur += Math.Abs(i.ParametroN ?? 0) * 3;
                    puntos.Add(new { tipo = "moonwalk", pasos = i.ParametroN });
                    break;
                case "combinada":
                    dur += 5;
                    puntos.Add(new { tipo = "combinada", raw = i.Raw });
                    break;
            }
        }
        ...
        return new SimulacionDto
        {
            ...
            duracion_estimada_seg = dur,
            distancia_total_cm    = Math.Round(dist, 2)
        };
    }
