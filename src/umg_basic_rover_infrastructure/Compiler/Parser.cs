...
public class Parser
{
    ...
    private Token Consumir(TokenType tipo, string valor, string msg, string sugerencia = "")
    {
        var t = Actual();
        if (t.Tipo == tipo && t.Lexema == valor) { Avanzar(); return t; }
        throw new ParseException(
            $"{msg} Se encontró '{t.Lexema}'.",
            t.Linea, t.Columna, sugerencia);
    }

    private Token ConsumirTipo(TokenType tipo, string msg, string sugerencia = "")
    {
        var t = Actual();
...
public class ParseException : Exception
{
    ...
    public ParseException(string message, int linea = 0, int columna = 0, string sugerencia = "")
        : base(message)
    {
        Linea      = linea;
        Columna    = columna;
